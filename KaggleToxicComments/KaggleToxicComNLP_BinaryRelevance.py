...
vectorizer = TfidfVectorizer(analyzer='char', ngram_range=(1,3), max_features=40000, min_df=2)
...
for i, j in enumerate(col):
    ...
    model.fit( data[:nrow_train], train_df[j])
    y_pred_X = model.predict(data[:nrow_train])
    print('Training accuracy is {}'.format(accuracy_score(train_df[j], y_pred_X)))
